...
describe("Execution Cases", () => {
    ...
    describe("basic", () => {
        ...
        test("create", async () => {
            const createNewExecCase = await axios.post(`${SERVER_URL}/admin/executioncases`, NEW_EXECUTION_CASE);
            expect(createNewExecCase.data).toMatchObject({ id: expect.any(Number) });
            execCaseId = createNewExecCase.data.id;
            const fetchNewExecCase = await axios.get(`${SERVER_URL}/admin/executioncases/${execCaseId}`);
        ...
        test("update", async () => {
            ...
            const updateExecCase = await axios.patch(`${SERVER_URL}/admin/executioncases/${execCaseId}`, UPDATED_EXECUTION_CASE)
            const getUpdatedExecCase = await axios.get(`${SERVER_URL}/admin/executioncases/${execCaseId}`);
            expect(getUpdatedExecCase.data).toMatchObject({ id: execCaseId, ...UPDATED_EXECUTION_CASE });
        });
        test("delete", async () => {
            expect(execCaseId).toBeDefined();
            const deleteExecCase = await axios.delete(`${SERVER_URL}/admin/executioncases/${execCaseId}`);
            const getDeletedExecCase = await axios.get(`${SERVER_URL}/admin/executioncases/${execCaseId}`, { validateStatus: () => true });
    ...
    describe("create - malformed payload", () => {
        ...
        async function noExecCaseCreated(newExecCaseData: Object) {
            ...
            const postPostListOfAllExecCases = await axios.get(`${SERVER_URL}/admin/executioncases`);
        ...
        test("extra data", async () => {
            ...
            execCaseId = createNewExecCase.data.id;
            expect(createNewExecCase.data).toMatchObject({ id: expect.any(Number) });
            const cleanUpCreatedExecCase = await axios.delete(`${SERVER_URL}/admin/executioncases/${execCaseId}`);
        });
    });
    ...
    describe("update - malformed payload", () => {
        ...
        afterEach(async () => {
            const deleteNewExecCase = await axios.delete(`${SERVER_URL}/admin/executioncases/${execCaseId}`);
        ...
        test("missing prop 1", async () => {
            ...
            const updateNewExecCase = await axios.patch(`${SERVER_URL}/admin/executioncases/${execCaseId}`, UPDATED_EXECUTION_CASE);
            const getUpdatedExecCase = await axios.get(`${SERVER_URL}/admin/executioncases/${execCaseId}`);
        ...
        test("missing necessary props", async () => {
            ...
            const initialExecCaseData = await axios.get(`${SERVER_URL}/admin/executioncases/${execCaseId}`);
            const updateExecCase = await axios.patch(`${SERVER_URL}/admin/executioncases/${execCaseId}`, UPDATED_EXECUTION_CASE);
            const postPatchExecCaseData = await axios.get(`${SERVER_URL}/admin/executioncases/${execCaseId}`);
    ...
    test("delete - no execution case exists", async () => {
        const initialExecCaseData = await axios.get(`${SERVER_URL}/admin/executioncases`);
        const deleteExecCase = await axios.delete(`${SERVER_URL}/admin/executioncases/-1`, { validateStatus: () => true });
        expect(deleteExecCase.status).toBe(500);
        const postPatchExecCaseData = await axios.get(`${SERVER_URL}/admin/executioncases`);
        expect(postPatchExecCaseData.data).toMatchObject(initialExecCaseData.data);
    });
});